...
pkgs.writeShellScriptBin "wall-select" ''
  ...
  # Special case: when 'aniwalls' is selected
  if [ "$selected_folder" == "$HOME/media/aniwalls" ]; then
      # Get all image files from aniwalls folder
      images=($(ls "$HOME/media/aniwalls" | grep -E '\.(jpg|jpeg|png|bmp|gif)$' | sort))

      # Check if there are any images in the aniwalls folder
      if [ ''${#images[@]} -eq 0 ]; then
          echo "No images found in the aniwalls folder."
          exit 1
      fi
      ...
      swww img "$HOME/media/aniwalls/$selected_image" --transition-type random --transition-fps 60
      echo "Wallpaper set to $HOME/media/aniwalls/$selected_image"
      echo "\$WALLPAPER=$HOME/media/aniwalls/$selected_image" > /tmp/.current_wallpaper_path_hyprlock
      echo "$HOME/media/aniwalls/$selected_image" > /tmp/.current_wallpaper_path
      exit 0
  fi

  # Special case: when 'anibg' is selected
  if [ "$selected_folder" == "wallhaven" ]; then
