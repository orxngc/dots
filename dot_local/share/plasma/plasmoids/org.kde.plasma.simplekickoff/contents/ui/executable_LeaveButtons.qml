...
RowLayout {
    ...
    Item {
        Layout.fillWidth: plasmoid.configuration.primaryActions === 3
    }
    ...
    Repeater {
        ...
        delegate: PC3.ToolButton {
            ...
            Keys.onLeftPressed: if (LayoutMirroring.enabled) {
                nextItemInFocusChain().forceActiveFocus(Qt.BacktabFocusReason)
            } else if (index > (plasmoid.configuration.primaryActions === 0 ? 3 : 0)) {
